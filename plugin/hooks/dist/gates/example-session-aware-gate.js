...
var session_1 = require("../hooks-app/dist/session");
/**
...
function execute(input) {
    return __awaiter(this, void 0, void 0, function () {
        var session, activeCommand, hasRustFiles, metadata, reminderCount;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    session = new session_1.Session(input.cwd);
                    return [4 /*yield*/, session.get('active_command')];
                case 1:
                    activeCommand = _a.sent();
                    if (activeCommand !== '/execute') {
                        return [2 /*return*/, { additionalContext: '' }];
                    }
                    return [4 /*yield*/, session.contains('file_extensions', 'rs')];
                case 2:
                    hasRustFiles = _a.sent();
                    if (!hasRustFiles) {
                        return [2 /*return*/, { additionalContext: '' }];
                    }
                    return [4 /*yield*/, session.get('metadata')];
                case 3:
                    metadata = _a.sent();
                    reminderCount = (metadata.rust_reminder_count || 0) + 1;
                    return [4 /*yield*/, session.set('metadata', __assign(__assign({}, metadata), { rust_reminder_count: reminderCount, last_rust_reminder: new Date().toISOString() }))];
                case 4:
                    _a.sent();
                    // Provide helpful context about Rust files
                    return [2 /*return*/, {
                            additionalContext: "\n## Rust Files Detected\n\nYou've edited Rust files during plan execution. Consider:\n- Dispatching rust-engineer agent for TDD workflow\n- Using rust-engineer for code review before completion\n- Ensuring Prime Directive (type system) compliance\n\n(Reminder #".concat(reminderCount, ")\n    ").trim()
                        }];
            }
        });
    });
